...
#'
#' @export
produce_de <- function(txi, design, contrasts, formula = ~ group, use_ruv = FALSE, ignoreTxVersion = TRUE, digits = 4, ncores = 1) {
    ...
    if (ncores == 1) {
        de_genes <- purrr::map(contrasts, ~ format_de(dds$genes, txi$genes, .x, ignoreTxVersion, digits = digits))
